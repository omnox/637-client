	...
	static final void decodeRegionOpcode(int i) {
		try {
			...
			if (i >= 4) {
				...
				int chunkCount = ((-IncomingMessages.packetStream.position + Class65.anInt496) / 16);
				Class98_Sub46_Sub17.regionKeys = new int[chunkCount][4];
				for (int localChunkX = 0; localChunkX < chunkCount; localChunkX++) {
					for (int localChunkY = 0; localChunkY < 4; localChunkY++) {
				...
				chunkCount = 0;
				for (int regionX = (-(GameWorld.size_x >> 4) + chunkX) / 8; ((GameWorld.size_x >> 4) + chunkX) / 8 >= regionX; regionX++) {
					for (int regionY = (chunkY - (GameWorld.size_y >> 4)) / 8; ((chunkY + (GameWorld.size_y >> 4)) / 8 >= regionY); regionY++) {
						Class121.anIntArray1006[chunkCount] = (regionX << 8) + regionY;
